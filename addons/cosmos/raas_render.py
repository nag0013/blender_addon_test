...
class RAAS_PT_simplify(RaasButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        if (
            context.scene.raas_cluster_presets_index > -1
            and len(pref.cluster_presets) > 0
        ):
            preset = pref.cluster_presets[context.scene.raas_cluster_presets_index]

            if (
                preset.raas_ssh_library == "ASYNCSSH"
                or preset.raas_ssh_library == "PARAMIKO"
            ):
                if (
                    len(preset.raas_da_username) == 0
                    or not pref.raas_scripts_installed
                    or len(pref.cluster_presets) < 1
                ):
...
class AuthenticatedRaasOperatorMixin:
    ...
    async def authenticate(self, context) -> bool:
        ...
        if (
            context.scene.raas_cluster_presets_index > -1
            and len(addon_prefs.cluster_presets) > 0
        ):
            preset = addon_prefs.cluster_presets[
...
FileType_items = [
    ...
    (
        ...
        "Select a .blend file together with directory with dependencies.",
    ),
]

MethodType_items = [("edge", "Edge", ""), ("seg", "Seg", "")]
...
class RAAS_PG_BlenderJobInfo(PropertyGroup):
    ...
    cosmos_prompt: bpy.props.StringProperty(
        name="Prompt",
    )  # type: ignore
...
class RAAS_UL_BlendFileJobs(bpy.types.UIList):
    """UIList that only shows jobs belonging to the currently open blend file."""
    ...
    def filter_items(self, context, data, propname):
        ...
        # Start with all items visible, then apply name and project filters.
        filtered = [self.bitflag_filter_item] * len(items)

        # Apply the standard name filter first.
        helpers = bpy.types.UI_UL_list
        name_filtered = helpers.filter_items_by_name(
            self.filter_name, self.bitflag_filter_item, items, "Name", reverse=False
        )
        if len(name_filtered) == len(items):
            filtered = name_filtered

        # Additionally hide items that don't belong to the current blend file.
        current_blend_stem = Path(context.blend_data.filepath).stem
        blend_project = (
            current_blend_stem.replace(" ", "_")
            .replace("\\", "_")
            .replace("/", "_")
            .replace("'", "_")
            .replace('"', "_")
        ...
        for i, item in enumerate(items):
            is_current_blend = False

            # Prefer exact metadata from local job.info (when available).
            if item.blender_job_info_json:
                try:
                    info = json.loads(item.blender_job_info_json)
                    stored_blendfile = info.get("blendfile", "")
                    stored_project = info.get("job_project", "")

                    if stored_blendfile:
                        stored_stem = Path(stored_blendfile).stem.replace(
                            ".braas-hpc", ""
                        )
                        if stored_stem == current_blend_stem:
                            is_current_blend = True

                    if not is_current_blend and stored_project == blend_project:
                        is_current_blend = True
                except Exception:
                    pass

            # Fallbacks for jobs without local metadata.
            if not is_current_blend:
                if item.Project == blend_project:
                    is_current_blend = True
                elif item.Name.endswith("-" + blend_project):
                    is_current_blend = True
...
class RAAS_PASSWORD_OT_input(bpy.types.Operator):
    ...
    server: bpy.props.StringProperty(name="Server")  # type: ignore
            ...
            try:
                ...
                if loop.is_running():
                    ...
                    future = asyncio.ensure_future(
                        session.asyncssh_create_session(
                            self.password, self.password_2fa
                        )
                    )
...
class RAAS_OT_download_files(
    async_loop.AsyncModalOperatorMixin, AuthenticatedRaasOperatorMixin, Operator
):
    """download_files"""
    ...
    async def async_execute(self, context):
        ...
        if idx != -1 and len(context.scene.raas_list_jobs) > 0:
            try:
                ...
                # Start file transfer
                fileTransfer = await raas_connection.start_transfer_files(
                    context, item.Id, self.token
                )

                # Download output files
                remote_storage_out = (
                    raas_connection.convert_path_to_linux(item.Name) + "/out"
                )
                local_storage_out = raas_connection.get_job_local_storage(item.Name)

                await raas_connection.transfer_files_from_cluster(
                    context,
                    fileTransfer,
                    remote_storage_out,
                    str(local_storage_out),
                    item.Id,
                    self.token,
                )
                ...
                # Download log files
                remote_storage_log = (
                    raas_connection.convert_path_to_linux(item.Name) + "/log"
                )
                ...
                remote_storage_job = (
                    raas_connection.convert_path_to_linux(item.Name) + "/job"
                )
                ...
                await raas_connection.transfer_files_from_cluster(
                    ...
                    fileTransfer,
                    remote_storage_job,
                    str(local_storage_job),
                    item.Id,
                    self.token,
                )

                # End file transfer
                await raas_connection.end_transfer_files(
                    context, fileTransfer, item.Id, self.token
                )
                ...
                self.report(
                    {"ERROR"},
                    "Problem with downloading files: %s: %s" % (e.__class__, e),
                )
                context.window_manager.raas_status = "ERROR"
                context.window_manager.raas_status_txt = (
                    "There is an error! Check Info Editor!"
                )
...
async def submit_job_save_blendfile(context, outdir):
    ...
    try:
        ...
        bpy.ops.wm.save_as_mainfile(filepath=str(filepath), compress=True, copy=True)
        ...
        try:
            ...
            if context.scene.raas_blender_job_info_new.file_type == "DEFAULT":
                if not os.path.exists(context.blend_data.filepath):
                    self.report(
                        {"ERROR"},
                        "Please save your Blend file before using the braas-hpc addon.",
                    )
                    context.window_manager.raas_status = "ERROR"
                    context.window_manager.raas_status_txt = (
                        "There is an error! Check Info Editor!"
            ...
            if (
                ...
                or context.scene.raas_blender_job_info_new.job_partition == ""
                or context.scene.raas_blender_job_info_new.job_allocation == ""
            ):
                self.report(
                    {"ERROR"},
                    "Select a configuration (cluster, partition, allocation).",
                )
                context.window_manager.raas_status = "ERROR"
                context.window_manager.raas_status_txt = (
                    "There is an error! Check Info Editor!"
            ...
            if (
                context.scene.raas_blender_job_info_new.job_project is None
                or len(context.scene.raas_blender_job_info_new.job_project) == 0
            ):
                context.scene.raas_blender_job_info_new.job_project = Path(
                    context.blend_data.filepath
                ).stem

            context.scene.raas_blender_job_info_new.job_project = (
                context.scene.raas_blender_job_info_new.job_project.replace(" ", "_")
                .replace("\\", "_")
                .replace("/", "_")
                .replace("'", "_")
                .replace('"', "_")
            ...
            if context.scene.raas_blender_job_info_new.file_type == "DEFAULT":
                ...
                filepath = Path(
                    raas_connection.get_blendfile_fullpath(context)
                ).with_suffix(".blend")

            if context.scene.raas_blender_job_info_new.file_type == "DEFAULT":
                # BAT-pack the files to the destination directory.
                missing_sources = await submit_job_bat_pack(
                    filepath,
                    context.scene.raas_blender_job_info_new.job_project,
                    outdir_in,
                )
                ...
                copy_tree(
                    bpy.path.abspath(
                        context.scene.raas_blender_job_info_new.blendfile_dir
                    ),
                    str(outdir_in),
                )
            ...
            context.scene.raas_blender_job_info_new.frame_start = (
                context.scene.frame_start
            )
            context.scene.raas_blender_job_info_new.frame_end = context.scene.frame_end
            context.scene.raas_blender_job_info_new.frame_current = (
            ...
            if missing_sources:
                ...
                self.report(
                    {"WARNING"},
                    "Raas job created with missing files: %s" % "; ".join(names),
                )

            # await raas_config.CreateJob(context, self.token)
            await context.scene.raas_config_functions.call_create_job(
                context, self.token
            )

            blender_job_info_new = context.scene.raas_blender_job_info_new

            local_storage_in = str(
                raas_connection.get_job_local_storage(blender_job_info_new.job_name)
            )
            ...
            fileTransfer = await raas_connection.start_transfer_files(
                context, submitted_job_info_ext_new.Id, self.token
            )
            await raas_connection.transfer_files_to_cluster(
                context,
                fileTransfer,
                local_storage_in,
                remote_storage_in,
                submitted_job_info_ext_new.Id,
                self.token,
            )
            await raas_connection.end_transfer_files(
                context, fileTransfer, submitted_job_info_ext_new.Id, self.token
            )
            ...
            self.report(
                {"ERROR"}, "Problem with submitting of job: %s: %s" % (e.__class__, e)
            )
            context.window_manager.raas_status = "ERROR"
            context.window_manager.raas_status_txt = (
                "There is an error! Check Info Editor!"
...
class RAAS_UL_ClusterPresets(bpy.types.UIList):
    ...
    def filter_items(self, context, data, propname):
        ...
        for i, item in enumerate(items):
            if (
                self.filter_name.lower() in item.allocation_name.lower()
                or self.filter_name.lower() in item.cluster_name.lower()
                or self.filter_name.lower() in item.partition_name.lower()
            ):
                filtered[i] |= self.bitflag_filter_item
...
class RAAS_PT_NewJob(RaasButtonsPanel, Panel):
    ...
    def draw(self, context):
        ...
        job_info_col.template_list(
            "RAAS_UL_ClusterPresets",
            "",
            addonprefs,
            "cluster_presets",
            context.scene,
            "raas_cluster_presets_index",
        )
        ...
        box.operator(
            RAAS_OT_submit_job.bl_idname, text="Submit Job", icon="RENDER_ANIMATION"
        )
...
async def GetCurrentInfoForJob(context, job_id: int, token: str) -> None:
    ...
    data = {"SubmittedJobInfoId": job_id, "SessionCode": token}
...
class RAAS_OT_GetCurrentInfoForJob(
    ...
    async def async_execute(self, context):
        ...
        try:
            ...
            self.report(
                {"ERROR"},
                "Problem with getting curent jobinfo: %s: %s" % (e.__class__, e),
            )
            context.window_manager.raas_status = "ERROR"
            context.window_manager.raas_status_txt = (
                "There is an error! Check Info Editor!"
...
async def GetUserGroupResourceUsageReport(context, token):
    ...
    resp_json = await raas_server.post(
        "JobReporting/GetUserGroupResourceUsageReport", data
    )
    pass
...
class RAAS_OT_GetUserGroupResourceUsageReport(
    ...
    async def async_execute(self, context):
        ...
        try:
            ...
            self.report(
                {"ERROR"}, "Problem with getting report: %s: %s" % (e.__class__, e)
            )
            context.window_manager.raas_status = "ERROR"
            context.window_manager.raas_status_txt = (
                "There is an error! Check Info Editor!"
...
async def ListSlurmJobsForCurrentUser(context, token):
    ...
    # Parse job data
    jobs_data = raas_jobs.slurm_parse_slurm_job_lines(
        res,
        context.scene.raas_blender_job_info_new.cluster_type,
...
async def ListPBSJobsForCurrentUser(context, token):
    ...
    # Parse job data
    jobs_data = raas_jobs.pbs_parse_pbs_job_lines(
        res,
        context.scene.raas_blender_job_info_new.cluster_type,
        context.scene.raas_blender_job_info_new.job_type,
    )
...
async def ListSchedulerJobsForCurrentUser(context, token):
    ...
    scheduler = context.scene.raas_config_functions.call_get_scheduler_from_context(
        context
    )
...
async def CancelJob(context, token):
    ...
    cmd = (
        "sed -i 's/job_state = R/job_state = C/g' %s/%s.job;sed -i 's/job_state = Q/job_state = C/g' %s/%s.job;echo '   ' ftime = $(date) >> %s/%s.job"
...
class RAAS_OT_CancelJob(
    ...
    async def async_execute(self, context):
        ...
        try:
            ...
            self.report(
                {"ERROR"}, "Problem with canceling of job: %s: %s" % (e.__class__, e)
            )
            context.window_manager.raas_status = "ERROR"
            context.window_manager.raas_status_txt = (
                "There is an error! Check Info Editor!"
...
class RAAS_OT_DeleteJob(
    ...
    async def async_execute(self, context):
        ...
        try:
            ...
            self.report(
                {"ERROR"}, "Problem with deleting of job: %s: %s" % (e.__class__, e)
            )
            context.window_manager.raas_status = "ERROR"
            context.window_manager.raas_status_txt = (
                "There is an error! Check Info Editor!"
    ...
    def draw(self, context):
        ...
        if idx != -1 and len(context.scene.raas_list_jobs) > 0:
            ...
            props = prop_btn_row.operator(
                RAAS_OT_explore_file_path.bl_idname, text="", icon="DISK_DRIVE"
            )
...
class RAAS_PT_ListJobsForBlendFile(RaasButtonsPanel, Panel):
    bl_label = "Current blend jobs"
    bl_parent_id = "RAAS_PT_simplify"
    ...
    def draw(self, context):
        ...
        row = layout.row()
        row.template_list(
            "RAAS_UL_BlendFileJobs",
            "",
            context.scene,
            "raas_list_jobs",
            context.scene,
            "raas_list_jobs_blendfile_index",
        )
        ...
        idx = context.scene.raas_list_jobs_blendfile_index
...
def register():
    ...
    scene.raas_cluster_presets_index = bpy.props.IntProperty(
        default=-1, options={"SKIP_SAVE"}
    )
    ################JobManagement#################
    scene.raas_list_jobs = bpy.props.CollectionProperty(
        type=RAAS_PG_SubmittedJobInfoExt, options={"SKIP_SAVE"}
    )
    scene.raas_list_jobs_index = bpy.props.IntProperty(
        default=-1, options={"SKIP_SAVE"}
    )
    scene.raas_list_jobs_blendfile_index = bpy.props.IntProperty(
        default=-1, options={"SKIP_SAVE"}
    )
    scene.raas_blender_job_info_new = bpy.props.PointerProperty(
        type=RAAS_PG_BlenderJobInfo, options={"SKIP_SAVE"}
    )
    scene.raas_submitted_job_info_ext_new = bpy.props.PointerProperty(
        type=RAAS_PG_SubmittedJobInfoExt, options={"SKIP_SAVE"}
    )
